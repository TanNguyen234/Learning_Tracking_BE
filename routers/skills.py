...
@router.get('/{skill_id}', status_code=status.HTTP_204_NO_CONTENT)
...
async def create_todo(user: user_dependency, db: db_dependency, skill_request: SkillRequest):
    check_user_authentication(db, user)
...
async def delete_user(user: user_dependency, db: db_dependency, skill_id: int = Path(gt=0)):
    check_user_authentication(db, user)
